...
class StandardRoIHeadOAMIL(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        # bbox_results 字典(['cls_score', 'bbox_pred', 'bbox_feats'])
        # bbox_results['cls_score'].shape)  # [512 * Batch, Class+1]   类别
        # bbox_results['bbox_pred'].shape)  # [512 * Batch, Class * 4] 坐标
        # bbox_results['bbox_feats'].shape) # [512 * Batch, 256, 7 ,7] 特征

        # print("******************StandardRoiHead_OAMIL1", len(rois))    # 【512 * Batch， 5】为类别所预测的框中最终选择出一个框，作为ROI

        # print("******************StandardRoiHead_OAMIL2", bbox_targets[0].shape)    # 【512 * Batch】   labels
        # print("******************StandardRoiHead_OAMIL3", bbox_targets[1].shape)    # 【512 * Batch】   label_weights
        # print("******************StandardRoiHead_OAMIL4", bbox_targets[2])          # 【512 * Batch,4】 bbox_targets
        # print("******************StandardRoiHead_OAMIL5", bbox_targets[3].shape)    # 【512 * Batch,4】 bbox_weights
        # apply OA-MIL
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, img_metas, **kwargs):
        ...
        bbox_results = self._bbox_forward(x, rois)
        bbox_targets = self.bbox_head.get_targets(sampling_results, gt_bboxes, gt_labels, self.train_cfg, **kwargs)
