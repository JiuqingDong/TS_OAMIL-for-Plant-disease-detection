...
xml_path = '/Users/jiuqingdong/Desktop/test/labels/unlabeled_xml'     # 你的xml文件路径
img_path = '/Users/jiuqingdong/Desktop/test/images/unlabeled'
img_xml = '/Users/jiuqingdong/Desktop/test/show_few_shot_b2/'

# xml_path = '/Users/jiuqingdong/Desktop/test/test_xml'     # 你的xml文件路径
# img_path = '/Users/jiuqingdong/Desktop/test/detect'         # 图像路径

# if 'wrong' in img_path:
#     img_xml = '/Users/jiuqingdong/Desktop/test/show_wrong'       # 显示标注框保存该文件的路径
# else:
#     img_xml = '/Users/jiuqingdong/Desktop/test/show_right'       # 显示标注框保存该文件的路径
...
for name in tqdm(os.listdir(xml_path)):
    ...
    if os.path.exists(image_name):
        ...
        if len(allObjects) == 0:
            ...
            _xml_name = _xml_name.replace('images/unlabeled', 'labels/unlabeled_xml')
        ...
        for i in range(len(allObjects)):    # 遍历xml标签，画框并显示类别。
            ...
            if objectName in ['blossom_end_rot', 'graymold', 'powdery_mildew', 'spider_mite', 'spotting_disease']:
                c = True
                break
        ...
        for i in range(len(allObjects)):    # 遍历xml标签，画框并显示类别。
            ...
            elif objectName == 'powdery_mildew':
                xmin = int(object.find('bndbox').find('xmin').text)
                ymin = int(object.find('bndbox').find('ymin').text)
                xmax = int(object.find('bndbox').find('xmax').text)
                ymax = int(object.find('bndbox').find('ymax').text)
            ...
            elif objectName == 'spotting_disease':
                xmin = int(object.find('bndbox').find('xmin').text)
                ymin = int(object.find('bndbox').find('ymin').text)
                xmax = int(object.find('bndbox').find('xmax').text)
                ymax = int(object.find('bndbox').find('ymax').text)
