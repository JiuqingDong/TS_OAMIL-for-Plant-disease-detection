...
def preprocess_dataset(prefix='./data/paprika_ssl/'):
    print('\npreprocess GWHD dataset ...')
    coco = CocoDataset()

    # load training data & save as pkl
    ann_file_train = prefix + 'annotations/instances_train_control_class.json'   # 修改
    anno_train = coco.load_annotations(ann_file_train)
    save_path_train = prefix + 'paprika_train_control_class.pkl'                 # 修改
...
if __name__ == '__main__':
    ...
    # save noisy anno
    anno_prefix = './data/paprika_Ssl/noisy_pkl/'
    os.makedirs(anno_prefix, exist_ok=True)
    ann_name = save_path_train.split('/')[-1].split('.')[0]
    out_ann_file = '{}{}_noise-r{:.1f}.pkl'.format(anno_prefix, ann_name, box_noise_level)   # 修改
