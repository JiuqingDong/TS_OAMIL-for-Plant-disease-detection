    ...
    for idx in range(data_len):
        ...
        if box_noise_level > 0:
            ...
            for i, bbox in enumerate(bboxes):
                if anno['labels'][i] in [5,6,7]:
                    x1, y1, x2, y2 = bbox
                    noisy_bboxes.append([x1, y1, x2, y2])

                    print('healthy, background and unknown',anno['labels'][i])
                else:
                    print('noise_class',anno['labels'][i])
                    ...
                    # noisy coordinates
                    noisy_x1, noisy_y1, noisy_x2, noisy_y2 = max(0, noisy_cx - noisy_w / 2), max(0,
                                                                                                 noisy_cy - noisy_h / 2), min(
                        noisy_cx + noisy_w / 2, img_w - 1), min(noisy_cy + noisy_h / 2, img_h - 1)
...
def preprocess_dataset(prefix='./data/'):
    print('\npreprocess GWHD dataset ...')
    coco = CocoDataset()

    # load training data & save as pkl
    ann_file_train = prefix + 'annotations/instances_train_control_class.json'
    anno_train = coco.load_annotations(ann_file_train)
    save_path_train = prefix + 'paprika_train_control_class.pkl'
...
if __name__ == '__main__':
    ...
    # save noisy anno
    anno_prefix = data_prefix + 'noisy_pkl/'
